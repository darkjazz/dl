...
from sklearn.metrics import accuracy_score as accf
from sklearn.metrics import normalized_mutual_info_score as nmif
from sklearn.metrics import adjusted_rand_score as arif
            ...
            class PrintACC(callbacks.Callback):
                ...
                def on_epoch_end(self, epoch, logs=None):
                    ...
                    print(' ' * 8 + '|==>  acc: %.4f,  nmi: %.4f  <==|'
                          % (accf(self.y, y_pred), nmif(self.y, y_pred)))
    ...
    def fit(self, x, y=None, maxiter=2e4, batch_size=256, tol=1e-3,
        ...
        for ite in range(int(maxiter)):
            if ite % update_interval == 0:
                ...
                y_pred = q.argmax(1)
                if y is not None:
                    acc = np.round(accf(y, y_pred), 5)
                    nmi = np.round(nmif(y, y_pred), 5)
                    ari = np.round(arif(y, y_pred), 5)
